    ...
    return (
      ...
      >
        <div className={cn('size-full', className)}>
          <ScrollArea ref={ref} className="relative h-full px-2 pb-10" dir={dir as Direction}>
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ...
        ref={ref}
        className={cn(
          'absolute left-1.5 h-full w-px rounded-md bg-primary py-3 duration-300 ease-in-out hover:bg-primary/50 rtl:right-1.5',
          className,
        )}
        {...props}
      />
...
const Folder = forwardRef<HTMLDivElement, FolderProps & React.HTMLAttributes<HTMLDivElement>>(
  ({ className, element, value, isSelectable = true, isSelect, children, ...props }, ref) => {
    const { direction, indicator, openIcon } = useTree();
    ...
    return (
      <AccordionPrimitive.Item {...props} value={value} className="relative h-full overflow-hidden">
        ...
        <AccordionPrimitive.Content className="relative h-full overflow-hidden text-sm" forceMount>
...
>(({ className, elements, expandAll = true, children, ...props }, ref) => {
